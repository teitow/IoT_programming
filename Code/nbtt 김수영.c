...
// led 16진수 값  이부분 추가 
unsigned char led_array[]= {0x22,0x44,0x11,0x88};
...
void led_on(int strikes, int balls, int outs, int homerun) {
    ...
    if (strikes > 0) led_data |= led_array[0]; // Green LEDs
    if (balls > 0) led_data |= led_array[1];   // Yellow LEDs
    if (outs > 0) led_data |= led_array[2];    // Red LEDs
    if (homerun > 0) led_data |= led_array[3]; // Blue LEDs
