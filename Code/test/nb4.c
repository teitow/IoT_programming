...
void blink_led();
void writeToFNDDevice(int number);
...
void writeToFNDDevice(int number) {
    ...
    fnd_data[3] = 0xC0 + (number % 10); // 1의 자리
    fnd_data[2] = 0xC0 + ((number / 10) % 10); // 10의 자리
    fnd_data[1] = 0xC0 + ((number / 100) % 10); // 100의 자리
    fnd_data[0] = 0xC0 + ((number / 1000) % 10); // 1000의 자리

    fnd_mtx = open(fnd, O_RDWR);
    if (fnd_mtx < 0) {
        printf("FND 디바이스 열기 실패\n");
        exit(0);
    }
    write(fnd_mtx, fnd_data, 4);
    close(fnd_mtx);
}
...
void init_devices() {
    ...
    clcds = open(clcd, O_RDWR);
    fnd_mtx = open(fnd, O_RDWR);
    if (dipsw < 0 || leds < 0 || dot_mtx < 0 || tactsw < 0 || clcds < 0 || fnd_mtx < 0) {
        printf("디바이스 열기 실패\n");
        exit(0);
    }
    close(dipsw);
...
void input_number(char* number, int digits) {
    ...
    while (idx < digits) {
        ...
        if (tactsw_value >= 1 && tactsw_value <= 9) {
            ...
            writeToFNDDevice(tactsw_value); // TACT 스위치 값을 FND에 출력
            idx++;
